...
Task: An initial engineering answer is PARTIAL: it can discuss RepoSourceEvidence, but cannot determine whether a ContextPackage reconstructed from a serialized Understanding still has authoritative coverage. Diagnose the missing context, request the smallest exact source context needed, and after that context is supplied state what can be validated and what remains unknown. Do not infer authority from parsing and do not edit the repository.
Phase: INITIAL
...
visible_unknowns: 2
visible_omissions: 0
claim_classes: FACT=0, INFERENCE=0, ASSUMPTION=0, HYPOTHESIS=0
included source subjects: 0

### Visible unknowns
- package-grounding: Serialized Understanding and ContextPackage records are present, but exact package source grounding cannot be established from parsing alone.
- decision-applicability: Current decision applicability basis is not part of the supplied diagnostic context.

### Visible omissions
- none

### Source-backed claim classes
- no source-backed claim is asserted for the initial partial package

### Context affordances
- package-grounding (COMPONENT): Request the live accepted M2b binding contract needed for this unresolved package-grounding gap; the exact RepoView identity remains the package basis.

### Gap-bound request guidance
projection_version: m2c-request-guidance-v1
...
- UNRESOLVED package-grounding: Serialized Understanding and ContextPackage records are present, but exact package source grounding cannot be established from parsing alone.
  - required semantic evidence: live-accepted-m2b-binding
  - request scope: repository-source / COMPONENT
