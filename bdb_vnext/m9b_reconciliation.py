...
def _load_plan(authority: Path, maintenance_id: str, expected_plan_sha256: str | None = None) -> dict[str, Any]:
    ...
    if set(value) != required or value.get("schema") != M9B_RECONCILIATION_PLAN_SCHEMA:
...
def _subject(
    *,
    ...
    if (
        client_plan.get("client_plan_sha256") != plan.get("client_plan_sha256")
        ...
        or client_plan.get("native_manifest_sha256") != plan.get("native_manifest_digest")
        or client_plan.get("browser_bundle_digest") != plan.get("browser_bundle_digest")
        or client_plan.get("native_manifest_path") != plan.get("candidate_native_manifest_path")
    ):
...
def _target_record(subject: Mapping[str, Any]) -> ActivationRecord:
    ...
    return ActivationRecord(
        ...
        state="ACTIVE",
        source_head=str(plan["candidate_source_head"]),
        source_tree=str(plan["candidate_source_tree"]),
        m9a_freeze_digest=current.m9a_freeze_digest,
        browser_bundle_digest=str(plan["browser_bundle_digest"]),
        native_manifest_digest=str(plan["native_manifest_digest"]),
        writer_enabled=True,
        intake_enabled=True,
    )
...
def prepare_post_active_reconciliation(
    *,
    ...
    with BootstrapLock(authority / "bootstrap.lock"):
        ...
        old = subject["m9b"]
        plan_payload = {
            "schema": M9B_RECONCILIATION_PLAN_SCHEMA,
            "maintenance_id": maintenance_id,
            "maintenance_plan_sha256": maintenance_sha,
            ...
            "candidate_client_plan_sha256": subject["maintenance"]["client_plan_sha256"],
            "candidate_native_manifest_path": subject["maintenance"]["candidate_native_manifest_path"],
            "browser_bundle_digest": subject["maintenance"]["browser_bundle_digest"],
            "native_manifest_digest": subject["maintenance"]["native_manifest_digest"],
...
def _revalidate_subject_for_plan(
    *,
    ...
    m3c = subject.get("m3c")
    if not all(isinstance(value, Mapping) for value in (maintenance, bootstrap, route_state, m3c)):
    ...
    expected = {
        ...
        "candidate_client_plan_sha256": maintenance.get("client_plan_sha256"),
        "candidate_native_manifest_path": maintenance.get("candidate_native_manifest_path"),
        "browser_bundle_digest": maintenance.get("browser_bundle_digest"),
        "native_manifest_digest": maintenance.get("native_manifest_digest"),
