...
def _fixture(tmp_path: Path, *, candidate_known_good: bool = True) -> dict[str, object]:
    ...
    # M11c client staging intentionally happens after the M11a coordinated
    # backup.  Client bytes are independently content-addressed by the M11c
    # client plan and may be discarded before activation without touching the
    # known-good M11a recovery subject.
    ...
    assert (runtime / "clients" / "client-plan.json").is_file()
...
def _plan(fixture: dict[str, object]) -> dict[str, object]:
    ...
    assert isinstance(client_plan, dict)
...
def _apply(fixture: dict[str, object], plan: dict[str, object]):
    return m11c._apply_cutover(
        authority_root=fixture["authority"],
        cutover_id="final-1",
...
def test_apply_resumes_same_cutover_after_client_gate_activating(tmp_path: Path) -> None:
    fixture = _fixture(tmp_path)
    plan = _plan(fixture)
...
def test_apply_resumes_after_external_switch_before_client_gate_finalize(tmp_path: Path) -> None:
    ...
    try:
        ...
        prepared_query = m11c.query_prepared_activation(
            authority_root=fixture["authority"], preparation_id="prep-final"
        )
        m11c._publish_external_activation(
            authority=Path(fixture["authority"]), plan=plan, prepared_query=prepared_query
        )
    finally:
        authority.close()
...
def test_apply_is_idempotent_after_exact_completed_cutover(tmp_path: Path) -> None:
    fixture = _fixture(tmp_path)
    plan = _plan(fixture)
    first = _apply(fixture, plan)
    second = _apply(fixture, plan)
