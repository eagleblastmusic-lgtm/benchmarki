...
def _plan(project_id: str, version: int = 1, *, statuses: tuple[str, str, str] = ("active", "pending", "pending"), supersedes: int | None = None) -> ProjectPlan:
    return validate_project_plan({
        ...
        **({"supersedes_version": supersedes} if supersedes is not None else {}),
        "milestones": [{"id": "m1", "title": "Delivery", "description": "bounded delivery", "status": "active"}],
        "tasks": [
            ...
            {"id": "t2", "milestone_id": "m1", "title": "Review", "description": "second", "status": statuses[1], "dependencies": ["t1"], "acceptance_criteria": ["manual:visual review"]},
...
def _fixture(tmp_path: Path) -> tuple[ProjectCatalog, ProjectExecutionCoordinator, str]:
    ...
    memory = ProjectMemoryStore(runtime, project_id); plan = memory.ensure_initial_plan(_plan(project_id))
